  ...
  return (
    <main>
      <Breadcrumb
        pageName="Faculty"
        url="/"
        description="Our experienced faculty members are dedicated to providing quality education and guiding students towards success."
      />
      <section className="relative z-1 my-6">
        <div className="container">
          <div className="flex flex-col lg:flex-row items-start lg:items-center gap-6 justify-between mb-13">
            {/* <!-- top bar left --> */}
            <div className="flex flex-col sm:flex-row items-center gap-5">
              <div className="flex items-center gap-2.5">
                <Button
                  aria-label="button for Faculty grid tab"
                  variant="ghost"
                  size="lg"
                  onClick={() => setViewType("grid")}
                  className={`${
                    viewType === "grid"
                      ? "bg-accent border-none hover:bg-accent"
                      : "border-border-blue hover:bg-transparent hover:border-accent hover:text-accent"
                  } flex items-center justify-center text-white rounded-md border ease-out duration-200 cursor-pointer`}
                >
                  <svg
                ...
                <Button
                  aria-label="button for Faculty list tab"
                  variant="ghost"
                  size="lg"
            ...
            <div className="w-full sm:w-auto flex flex-col sm:flex-row items-center gap-5">
              ...
              <div className="relative w-full sm:w-64">
                <TechIcon
                  name="filter"
                  className="absolute left-3 top-1/2 transform -translate-y-1/2 text-border-gray h-5 w-5"
                />
                <select
                  value={selectedDepartment}
                  onChange={(e) => setSelectedDepartment(e.target.value)}
                  className="w-full pl-10 py-3 bg-bg text-soft-white border rounded-md outline-none transition-all duration-300 focus:border-transparent focus:ring-2 focus:ring-accent"
                >
          ...
          {viewType === "grid" ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
              {filteredFaculty.length > 0 ? (
                filteredFaculty.map((faculty) => (
                  <SingleGridItem key={faculty.id} faculty={faculty} />
                ))
              ) : (
                <div className="col-span-full h-[40vh] my-50">
                  <Empty />
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col gap-6">
              {filteredFaculty.length > 0 ? (
                filteredFaculty.map((faculty) => (
                  <SingleListItem key={faculty.id} faculty={faculty} />
                ))
              ) : (
                <div className="h-[40vh] my-50">
                  <Empty />
                </div>
              )}
            </div>
